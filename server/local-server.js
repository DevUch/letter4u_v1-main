...
function startPublicStatsRealtimeSync() {
  ...
  return function stopPublicStatsRealtimeSync() {
    clearWriteTimer();
    usersRef.off("value");
  };
}
...
function startServer() {
  ...
  app.get("/api/health", (_req, res) => {
    res.json({ ok: true, mode: "admin-sdk-local" });
  });

  app.post("/api/pages/check-id", async (req, res) => {
    try {
  ...
  app.post("/api/credits/voucher/check", async (req, res) => {
    try {
  ...
  app.post("/api/credits/ensure", requireAuth, async (req, res) => {
    try {
  ...
  app.post("/api/credits/increment", requireAuth, async (req, res) => {
    try {
  ...
  app.post("/api/credits/support-claim", requireAuth, async (req, res) => {
    try {
  ...
  app.post("/api/pages/save", requireAuth, async (req, res) => {
    try {
  ...
  app.post("/api/pages/delete", requireAuth, async (req, res) => {
    try {
  ...
  app.get("/api/admin/config/support-gate", requireAuth, requireAdmin, async (_req, res) => {
    try {
  ...
  app.post("/api/admin/config/support-gate", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/users/credits/adjust", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.get("/api/admin/pages/:pageId", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.delete("/api/admin/pages/:pageId", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.get(["/viewer/:id", "/viewer.html/:id"], (_req, res) => {
    res.sendFile(path.join(workspaceRoot, "viewer.html"));
  });
  ...
  const port = Math.max(1, toSafeInt(process.env.PORT, 3000));
  app.listen(port, () => {
    console.log(`[admin-api] Letter4U running on http://localhost:${port}`);
  });
}
